...
const PaymentBox = () => {
  ...
  const handlePayment = () => {
    ...
    alert(`Paiement de ${finalQuantity} verre(s) pour ${finalQuantity * pricePerGlass} FCFA\nMessage: ${message}`);
  };
    ...
    <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-xl border text-center text-black mt-10">
      ...
      <div className="flex justify-center gap-4 mb-6">
        {predefinedOffers.map((offer) => (
          <Button 
            key={offer} 
            className={`px-4 py-2 rounded-lg font-semibold border ${quantity === offer ? 'bg-black text-white' : 'bg-gray-100 text-black'}`}
            onClick={() => {
              setQuantity(offer);
      ...
      <div className="flex items-center justify-center gap-6 mb-6">
        <Button 
          ...
          onClick={() => setCustomQuantity((prev) => Math.max(1, (prev || quantity) - 1))}
        >
          <Minus size={20} />
        </Button>
      ...
      <p className="text-xl mb-6 font-semibold">
        Total : <span className="font-bold">{(customQuantity || quantity) * pricePerGlass} FCFA</span>
      </p>
